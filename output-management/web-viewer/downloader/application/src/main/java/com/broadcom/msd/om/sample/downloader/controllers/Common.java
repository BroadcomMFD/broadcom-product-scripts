...
public abstract class Common {
   ...
   * @param onDone Optional function executed when done. First argument indicates task success.
   * Second parameter contains the task return value on success and {@code null} otherwise.
   * @param <T> Task type.
   */
  protected <T> void runTask(String operation, Task<T> task, BiConsumer<Boolean, T> onDone) {
    ...
    final Consumer<Boolean> restore = success -> {
      ...
      if (onDone != null) {
        try {
          onDone.accept(success, success ? task.get() : null);

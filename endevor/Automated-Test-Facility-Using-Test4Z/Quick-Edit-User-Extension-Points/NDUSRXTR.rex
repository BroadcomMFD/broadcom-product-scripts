/* REXX - User Routine Sample to submit a TEST4Z RECORD job.                    
   Kick off a Test4Z job                                                        
*/                                                                              
Trace Off                                                                       
                                                                                
CALL BPXWDYN "INFO FI(NDUSRXTZ) INRTDSN(DSNVAR) INRDSNT(myDSNT)"                
...
ADDRESS ISPEXEC "VPUT (USERDAT1 USERDAT2",  /* Make vars available            */
 ...
 T4ZLDATA= Substr(LOADLIB,1,uptoLastnode) || 'T4ZLDATA'                         
 T4ZLDATA= T4ZLibraryPrefix'.'EEVETKSY'.'EEVETKEL'.T4ZLDATA'                    
                                                                                
ADDRESS ISPEXEC "ADDPOP"                  /* Show the panel in a pop-up       */
ADDRESS ISPEXEC "DISPLAY PANEL(NDUSRPTR)" /* Then show the panel              */
...
do                                                                              
 ...
 IF DSNCHECK /= 'OK' then,                                                      
    Do                                                                          
    Say "NDUSRXTZ- Expecting a member named" EEVETKEL "in" MYJCLLIB '.'         
    Say "NDUSRXTZ- Not finding one and cannot submit a TEST4Z job. "            
    Exit(8)                                                                     
    End                                                                         
...
ADDRESS ISPEXEC "VGET (USERDAT1 USERDAT2",  /* Get the values again           */
   ...
   ADDRESS ISPEXEC "FTINCL T4ZRCORD"                                            
   ...
   X = OUTTRAP("OFF")                                                           
   ADDRESS ISPEXEC "VPUT (MYJCLLIB)  PROFILE"                                   
...
/*                                                                   */         
  ...
  /*                                                                */          
    ...
    If LOADLIB = '' then,                                                       
       Do                                                                       
       Say "NDUSRXTZ- Unable to find output load library"                       
       Say "NDUSRXTZ- Bypassing the submit for a TEST4Z job. "                  
       Exit(8)                                                                  
       End                                                                      

...
/* Time = TIME()
   ...
   If rulTimeZone /= ' ' then,
      Do
      fullTime = Substr(rulTime,1,2) || ":" ||,
                 Substr(rulTime,3,2) || ":00"
      whattime = DTADJUST(rulTimeZone fullTime rulAdjustDate)
      Date = Word(whattime,1)
      Time = Word(whattime,2)
      Time = Substr(Time,1,2) || Substr(Time,4,2)
      End /* If rulTimeZone /= ' '  */
   Else,
      Do
...
*/
     ...
     Drop TimeZone
     rulTimeZone   = Value.SHIPRULE.TimeZone.rul#

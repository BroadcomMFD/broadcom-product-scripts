/*   rexx    */
  /* This REXX program drives the SonarQube Analyisis         */
  /* from an Endevor CAST action for a package containing     */
  ...
  If USERID() /= 'IBMUSER' &,
     USERID() /= 'IBMothr' then,
       DO
       Say 'SONRQUBE- the ' USERID(),
           'is not configured by SONRQUBE (REXX)',
                    'to invoke a SonarQube Analysis'
       Exit
       EnD
  */
...
ProcessPackageSCL:
 ...
 /* Use TableTool to create multiple outputs                 */
  ...
  QUEUE " HEADING1  TBLOUT1  SONAROPT 1    "
  ...
  STRING="ALLOC DD(SONAROPT) DA("SonarWorkfile"(SONAROPT)) SHR REUSE"
  CALL BPXWDYN STRING;
  ...
  Queue "End;                                                     "
  Queue "If HaveSonar = 0 then $SkipRow='Y'                       "
  ...
  CALL BPXWDYN STRING;
  Queue "  &Type    &Element   &SonarElmDSN "
  ...
  If myRC = 1 then SonarElementfound = 'Y'
  CALL BPXWDYN "FREE DD(SCL)     " ;
  CALL BPXWDYN "FREE DD(RESULTS) " ;
  CALL BPXWDYN "FREE DD(TABLE)   " ;
  CALL BPXWDYN "FREE DD(PARMLIST)" ;
/* SONAROPT is not freed so that it can be used after CAST
   to find the SonarQube analysis report and append it to the
   request-for-approval Emails
  CALL BPXWDYN "FREE DD(SONAROPT) " ;     */
  ...
  /* append Trailer2  to TBLOUT2  */
  "EXECIO * DISKR TBLOUT2 (stem apicall. Finis"
  "EXECIO * DISKR TRAILER2 (stem trailer. Finis"
  "EXECIO" apicall.0 "DISKW TBLOUT2 (stem apicall. "
  "EXECIO" trailer.0 "DISKW TBLOUT2 (stem trailer. Finis"
  CALL BPXWDYN "FREE DD(TBLOUT2) " ;
  CALL BPXWDYN "FREE DD(TRAILER2)" ;
  RETURN ;
RETRIEVE_Sonar_Elements:
...
Use_ENTBJAPI_For_BX_Info:
  ...
  STRING = "ALLOC DD(COMPLIST) LRECL(200) BLKSIZE(20000) ",
             ...
             " MOD UNCATALOG REUSE ";
  CALL BPXWDYN STRING;
  /* Call Built-in API program for Sonar source extracts  */
  /* and component List requests.                         */
...
ProcessCopybookMembers:
   Do rec# = 1 to cmplist.0
      ...
      If Substr(Word(complist,5),3,1) = ':' &,
         WordPos(StepName,GenerateProcessorStepnames) > 0 &,
         DDname  = 'SYSLIB'     then,
         Do
...
SubmitandWaitForSonarQube:
  ...
  */
  STRING="ALLOC DD(SETUP) DA("SonarWorkfile"(SONAROPT)) SHR REUSE"
  CALL BPXWDYN STRING;
  ...
  If Wait_for_SonarQube = 'Y' then,
     ...
     Say 'SONRQUBE - Submitting and waiting',
         'for a SONARQUBE analysis'
     End
  Else,  /* exclude entries that cause waiting */
     Do
     "EXECIO 6 DISKW PARMLIST (FINIS ";
     Say 'SONRQUBE - Submitting and not waiting',
         'for a SONARQUBE analysis'
     End
  ...
  If Wait_for_SonarQube /= 'Y' then Return
  ...
  If myRC < 8 then,
     ...
     If RC > 4 then,
        Do
        Message = 'SONRQUBE - SonarQube analysis was not',
                  ' returned'
        myRc= 8
        Return
        End
     ...
     If RC > 4 then,
        Do
        Message = 'SONRQUBE - SonarQube analysis cannot ',
                  'be read'
        myRc= 8
        Return
        End

...
/*                                                                  */
  ...
  Arg Parameters;
  Format = 'Table'      /* Table or Rexx */
  If Words(Parameters) > 1 then Format = Word(Parameters,2)
  SCLDataset = Word(Parameters,1)

  If Pos('.',SCLDataset) = 0 & Words(Parameters) = 1 then,
     Format = Word(Parameters,1)
  If Pos('.',SCLDataset) > 0 then,
     CALL BPXWDYN,
       "ALLOC DD(SCL) DA("SCLDataset") SHR REUSE"

  "EXECIO * DISKR SCL (Stem scl. Finis"
  If RC > 4 then,
     Do
     Say 'SCAN#SCL- unable to find input SCL'
     Exit(8)
     End
  If scl.0 > 0 then,
     Do
...
/* MG added Translate */
  ...
  If Length(C1Element) > 10 then,
     C1Element = Translate(C1Element,'_',' ');

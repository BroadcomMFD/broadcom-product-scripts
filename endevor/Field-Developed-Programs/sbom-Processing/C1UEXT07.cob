      ...
      * 1) PERFORM SBOM processing at CAST time                  *
      ...
      *
       ...
       01  EXECBLK.
           ...
           05 EXECBLK-RESERVED               PIC S9(8) BINARY
                                                       VALUE 0.
           05 EXECBLK-MEMBER                 PIC X(08) .
              88 TIME-TO-BUILD-SBOM          VALUE 'PKGESBOM'.
              88 TIME-TO-EXECUTE-PKG         VALUE 'PKGEXECT'.
      ...
      * IRXJCL or IRXEXEC call to the rexx program PKGESBOM/PKGEXECT.
      * IRXJCL is used when running in batch (batch CAST) .
      * IRXEXEC is used when running in foreground (CAST or APPROVE).
       01  PKG-CAST-PARMS-IRXJCL.
         02  PKG-CAST-PARMS-IRXJCL-TOP.
           03 PARM-LENGTH          PIC X(2) VALUE X'0114'.
           03 REXX-NAME            PIC X(8) VALUE 'PKGESBOM'.
           03 FILLER               PIC X(1) VALUE SPACE .
         02  PKG-CAST-PARMS-IRXEXEC.
           03 REXX-CAST-PACKAGE    PIC X(16) .
           03 REXX-CAST-AUTOCAST   PIC X(1) .
           03 REXX-CAST-ENV        PIC X(08) .
           03 FILLER               PIC X(1) VALUE SPACE .
           03 REXX-CAST-STGID      PIC X(01) .
           03 REXX-CAST-MODE       PIC X(01) .
           03 REXX-CAST-CREATE-USER PIC X(08) .
           03 REXX-CAST-UPDATE-USER PIC X(08) .
           03 REXX-CAST-CAST-USER  PIC X(08) .
           03 REXX-CAST-COMMENT    PIC X(50) .
...
**   *******====---> SUBMIT PACKAGE SHIPMENT JOBS
           IF CAST-PACKAGE AND PECB-BEFORE AND
              ...
              PERFORM 300-CREATE-PACKAGE-SBOM
...
*******    DISPLAY 'C1UEXT07:   GOING BACK '
       ...
       300-CREATE-PACKAGE-SBOM.

      *    MAKES A CALL TO THE REXX ROUTINE PKGESBOM.
      *    THE REXX ROUTINE PKGESBOM creates an SBOM for the package
      *    if one is not already there.

********   IF PECB-USER-BATCH-JOBNAME(1:7) = ME
********       DISPLAY 'C1UEXT07: processing SBOM    '
********               PECB-PACKAGE-ID
********   END-IF

           MOVE PECB-PACKAGE-ID      TO REXX-CAST-PACKAGE
           MOVE PECB-AUTOCAST        TO REXX-CAST-AUTOCAST
           MOVE PHDR-PKG-ENV         TO REXX-CAST-ENV
           MOVE PHDR-PKG-STGID       TO REXX-CAST-STGID
           MOVE PECB-MODE            TO REXX-CAST-MODE
           MOVE PHDR-PKG-CREATE-USER TO  REXX-CAST-CREATE-USER
           MOVE PHDR-PKG-UPDATE-USER TO  REXX-CAST-UPDATE-USER
           MOVE PHDR-PKG-CAST-USER   TO  REXX-CAST-CAST-USER
           MOVE PREQ-PACKAGE-COMMENT TO REXX-CAST-COMMENT
           MOVE 'PKGESBOM'           TO EXECBLK-MEMBER .
...
********      DISPLAY 'C1UEXT07: IN TSO FOREGROUND  '
              CALL 'SET-ARG1-POINTER'  USING ARGUMENT-PTR
                                             PKG-CAST-PARMS-IRXEXEC
              PERFORM 1800-REXX-CALL-VIA-IRXEXEC
           ELSE
********      DISPLAY 'C1UEXT07: NOT IN TSO FOREGROUND  '
              CALL IRXJCL  USING PKG-CAST-PARMS-IRXJCL
********      DISPLAY 'C1UEXT07: RETURN-CODE=' RETURN-CODE
           END-IF.

           IF RETURN-CODE > 4
               MOVE 'C1UEXT07: Package is without an sbom'
                        TO PECB-MESSAGE

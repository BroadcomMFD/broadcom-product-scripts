...
//*-------------------------------------------------------------------*
// JCLLIB  ORDER=(SYSDE32.NDVR.TEAM.JCL)      <-Where is STEPLIB incl
//*==================================================================*
//  EXPORT SYMLIST=(*)
//*-------------------------------------------------------------------*
...
//OPTIONS  DD  *
  ...
  Break = 30
  Stoppoint = Break - 1;
  if $row# = 0 then #count =  0;
  if $row# = 0 then #Grp =  1;
  #count = #count + 1 ;
  if #count = 1 then START = ELEMENT
  if #count = 1 then #Grp  = #Grp + 1;
  if #count // Break = 1 then START = ELEMENT
  if #count // Break = 0 then STOP = ELEMENT
  if #count // Break = 0 then TBLOUT= "TBLOUT"
  if $tbl >= $tablerec.0 then STOP = ELEMENT
  if $tbl >= $tablerec.0 then TBLOUT= "TBLOUT"
  if $tbl >= $tablerec.0 then #count = #count + Break ;
  GROUP = "GRP"#count % Break
